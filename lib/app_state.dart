...
class AppState extends ChangeNotifier {
  ...
  bool _isReconcilingProfiles = false;
  bool _hasReconciledProfiles = false;
  ...
  Future<String?> login({
    ...
    if (_usesFirebase) {
      try {
        ...
        await _deactivateProfileIfAuthMissing(normalizedEmail, error.code);
...
Generated By: Shramik App''';
  ...
  void _bindFirebaseStreams() {
    ...
    _usersSubscription = _usersCollection.snapshots().listen((snapshot) {
      ...
      if (!_hasReconciledProfiles && !_isReconcilingProfiles) {
        unawaited(_reconcileActiveProfiles());
      }
      notifyListeners();
    });
  ...
  Future<void> _refreshCurrentUser(String uid) async {
    ...
    if (cachedUser != null) {
      if (!cachedUser.isActive) {
        await _setUserActiveState(cachedUser.id, true);
        _currentUser = cachedUser.copyWith(isActive: true);
      } else {
    ...
    if (!user.isActive) {
      await _setUserActiveState(user.id, true);
      _currentUser = user.copyWith(isActive: true);
      return;
    }
    _currentUser = user;
  }
  ...
  Future<void> _deactivateProfileIfAuthMissing(
    String email,
    String errorCode,
  ) async {
    ...
    if (errorCode != 'user-not-found' && errorCode != 'invalid-credential') {
      return;
    }
    ...
    final accountExists = await _authAccountExistsByEmail(email);
    if (accountExists) {
      return;
    }

    await _setUserActiveState(candidate.id, false);
  }

  Future<bool> _authAccountExistsByEmail(String email) async {
      ...
      request.add(
        utf8.encode(
          jsonEncode(<String, dynamic>{
            'email': email,
            'password': '__probe_invalid_password__',
  ...
  Future<void> _setUserActiveState(String userId, bool value) async {
    if (_usesFirebase) {
  ...
  Future<void> _reconcileActiveProfiles() async {
    if (_isReconcilingProfiles || !_usesFirebase) {
      return;
    }

    _isReconcilingProfiles = true;
    try {
      final usersToCheck = _users
          .where((user) => user.role != UserRole.admin)
          .toList(growable: false);

      for (final user in usersToCheck) {
        final exists = await _authAccountExistsByEmail(user.email);
        if (user.isActive != exists) {
          await _setUserActiveState(user.id, exists);
        }
      }
      _hasReconciledProfiles = true;
    } finally {
      _isReconcilingProfiles = false;
    }
  }
